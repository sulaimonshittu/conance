...
const ArtisanNavBar = () => {
    const { userDetails, logout, isLoading } = useAuth()
            ...
            <div className="relative">
                ...
                {isMenuOpen && (
                    <>
                        ...
                        />
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-accent z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                            <div className="p-s2 border-b border-accent bg-slate-50/50">
                                ...
                                <p className="text-b3 font-bold text-gray-900 truncate">{userDetails?.email}</p>
                            </div>
                            <button
                                onClick={() => {
                                    setIsMenuOpen(false)
                                ...
                                disabled={isLoading}
                                className="w-full flex items-center gap-3 px-s2 py-s2 text-red-600 hover:bg-red-50 transition-colors text-b3 font-bold disabled:opacity-50"
                            >
                                <LogOut size={18} />
